...
pub async fn search(&self, query: &str) -> anyhow::Result<Vec<VideoMeta>> {
    if let Some(cached) = self.cache.get::<Vec<VideoMeta>>(query) {
        return Ok(cached);
    }
    let fresh = self.fetch_from_api(query).await?;
    self.cache.put(query, &fresh)?;
    Ok(fresh)
}
